...
MODELS_TO_TEST = [
    ...
    },
    {
        ...
        "base_url": "https://openrouter.ai/api/v1",
        "api_key": OPENROUTER_API_KEY,
        "display_name": "OpenRouter Llama3.3-70B",
        "rpm_limit": 20,
    },
]
...
def call_llm(base_url: str, api_key: str, model: str, prompt: str, max_retries: int = 3) -> Dict[str, Any]:
    """调用 LLM API，带重试机制"""
    headers = {
    ...
    for attempt in range(max_retries):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                # 估算token数
                tokens = int(len(content) * 1.5 if any('一' <= c <= '鿿' for c in content) else len(content) / 4)
                ...
                print(f"      ⚠️ 429限流，等待 {wait_time}s 后重试 ({attempt+1}/{max_retries})...")
...
def test_model(model_config: Dict) -> ModelBenchmark:
    ...
    print(f"RPM限制: {model_config['rpm_limit']}")
    print(f"{'='*80}")

    # 计算安全的请求间隔（避免429）
    # 每个问题间隔 = 60秒 / RPM限制 * 1.5（安全系数）
    safe_interval = 60 / model_config['rpm_limit'] * 1.5
    print(f"请求间隔: {safe_interval:.1f}s")
    ...
    for i, q in enumerate(TEST_QUESTIONS):
        ...
        # 第一个问题之前的等待（除了第一个）
        if i > 0:
            print(f"      ⏳ 等待 {safe_interval:.1f}s 避免限流...")
...
def generate_report(results: List[ModelBenchmark]) -> str:
    ...
    report.append(f"**限流策略**: 根据各API的RPM限制动态调整请求间隔\n")
...
def main():
    """主函数"""
    print("="*80)
    print("免费 LLM 模型全面评测系统 v2")
    print(f"测试问题: {len(TEST_QUESTIONS)} 个")
    print(f"评估维度: {len(EVAL_DIMENSIONS)} 个")
    print(f"测试模型: {len(MODELS_TO_TEST)} 个")
    print("限流策略: 根据RPM动态调整请求间隔，遇429自动重试")
    print("="*80)

    # 预计时间
    total_time = 0
    for model in MODELS_TO_TEST:
        interval = 60 / model['rpm_limit'] * 1.5
        total_time += interval * 9  # 10个问题，9个间隔
        total_time += 30  # 每个模型的请求时间
    print(f"\n预计总时间: {total_time/60:.1f} 分钟")
    ...
    # 保存报告
    report_path = os.path.join(os.path.dirname(__file__), "..", "docs", "free_model_benchmark_report_v2.md")
    with open(report_path, "w", encoding="utf-8") as f:
        f.write(report)

    # 保存JSON数据
    json_path = os.path.join(os.path.dirname(__file__), "..", "docs", "free_model_benchmark_data_v2.json")
    json_data = {
