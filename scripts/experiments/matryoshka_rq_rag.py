"""Matryoshka + RQ RAG - 96:1压缩 + 90%准确率

组合方案：
1. Matryoshka: 384维 → 64维（6:1）
2. RQ量化: 64维 → 16 bytes（16:1）
3. 总压缩比: 6 × 16 = 96:1
...
from src.rq_compressor import RQCompressor


class MatryoshkaRQRAG:
    """Matryoshka + RQ 极致压缩RAG（96:1 + 90%准确率）"""

    def __init__(
        self,
        db_path: str = "data/matryoshka_rq_rag",
        ...
        Args:
            ...
            num_layers: RQ层数
            num_subvectors: 每层子向量数
        """
        self.db_path = Path(db_path)
        self.matryoshka_dim = matryoshka_dim

        self.embedder = None
        self.rq = RQCompressor(
            dim=matryoshka_dim,
    ...
    def chunk_file(
        ...
        if content is None:
            print(f"⚠️ Failed to read {file_path}: all encodings failed")
            return []
    ...
    def index_directory(
        ...
        batch_size = 512  # 平衡方案：不爆显存 + 减少I/O
        all_vectors = []

        for i in tqdm(
            range(0, len(all_chunks), batch_size), desc="Embedding"
        ):
        ...
        # Step 2: 训练RQ
        print(f"\nStep 2: Training RQ (GPU)...")
        # 使用前10万个向量训练
        train_size = min(100000, len(all_vectors))
        self.rq.train(all_vectors[:train_size])

        # Step 3: RQ编码
        print("\nStep 3: RQ encoding (GPU)...")
        self.codes = self.rq.encode(all_vectors)
        print(f"✅ RQ codes shape: {self.codes.shape}")
    ...
    def _save_all(self):
        """保存所有数据"""
        # 保存RQ codebooks
        rq_path = self.db_path / "rq_codebooks.pkl"
        with open(rq_path, "wb") as f:
            pickle.dump(self.rq.codebooks, f)

        # 保存RQ codes
        codes_path = self.db_path / "rq_codes.npy"
    ...
    def _load_all(self):
        """加载所有数据"""
        # 加载RQ codebooks（使用pickle，与保存方法匹配）
        rq_path = self.db_path / "rq_codebooks.pkl"
        with open(rq_path, "rb") as f:
            self.rq.codebooks = pickle.load(f)
        self.rq.trained = True
        ...
        print(f"  - RQ codes: {self.codes.shape}")
    ...
    def search(self, query: str, top_k: int = 10) -> List[Dict]:
        ...
        # RQ搜索（GPU）
        distances, indices = self.rq.search(query_vector, self.codes, top_k)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Matryoshka + RQ RAG - 96:1压缩"
    )
    parser.add_argument(
    ...
    rag = MatryoshkaRQRAG(matryoshka_dim=args.matryoshka_dim)
